"""
Enrich AtCoder problem statements using LLM normalization.
...
Usage:
    ...
    poetry run python -m scripts.enrich_atcoder_tasks -i scripts/atcoder_parser/output -o scripts/atcoder_parser/output_enriched
    poetry run python -m scripts.enrich_atcoder_tasks --in-place        # overwrite input files
    poetry run python -m scripts.enrich_atcoder_tasks --skip-existing   # skip already enriched
    poetry run python -m scripts.enrich_atcoder_tasks -n 10             # enrich at most 10 tasks
"""
...
DEFAULT_INPUT = "scripts/atcoder_parser/output"
DEFAULT_OUTPUT = "scripts/atcoder_parser/output_enriched"
DEFAULT_DELAY = 0.5

# AtCoder statement sections to feed the enricher, in order.
# Samples (Sample Input/Output N) are intentionally excluded.
_STATEMENT_SECTIONS = ("Problem Statement", "Constraints", "Input", "Output")
...
def _build_statement_text(problem: dict) -> str:
    ...
    for section_name in _STATEMENT_SECTIONS:
        text = sections.get(section_name, "")
        if isinstance(text, list):  # tolerate list-shaped sections
            text = "\n".join(text)
        text = text.strip()
        if text:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Enrich AtCoder problems with LLM")
    parser.add_argument(
        "-i", "--input",
